...
import { ArrowLeft, BookOpen, Code, Award } from "lucide-react"

import LessonVideo from "@/components/module/lesson-video"
import LessonReading from "@/components/module/lesson-reading"
...
export default function LessonPage({ params }: { params: { id: string; lessonId: string } }) {
  ...
  // Simulate fetching course data (duplicated for simplicity, ideally fetched once)
                ...
                },
                {
                  ...
                  content: [
                    ...
                    },
                    {
                      type: "paragraph",
                      value:
                        "Para **Delete (DELETE)**, se define una ruta con el ID del recurso (ej. `/api/productos/:id`) para eliminarlo. La respuesta puede ser un código 204 (No Content) si la eliminación fue exitosa y no hay contenido que devolver.",
                    },
                    {
                      type: "paragraph",
                      value:
  ...
  const handleNext = (isCorrect?: boolean) => {
    ...
    const lessonTokens = 10 // Example: 10 tokens per lesson
    const storedUser = localStorage.getItem("learnquest_user")
    if (storedUser) {
      const user = JSON.parse(storedUser)
      user.totalTokens = (user.totalTokens || 0) + lessonTokens

      // Update progress for the current course
      if (!user.purchasedCourses) {
        user.purchasedCourses = []
      }
      const courseProgressIndex = user.purchasedCourses.findIndex((c: any) => c.id === courseId)
      if (courseProgressIndex !== -1) {
        const totalLessonsInCourse = allLessons.length
        const completedLessons = currentLessonIndex + 1
        user.purchasedCourses[courseProgressIndex].progress = Math.round(
          (completedLessons / totalLessonsInCourse) * 100,
        )
      } else {
        // If course not in purchasedCourses, add it with initial progress
        user.purchasedCourses.push({
          id: courseId,
          title: courseData.title,
          progress: Math.round((1 / allLessons.length) * 100),
          isCompleted: false,
        })
      }
      ...
      user.recentActivity.unshift({
        title: `Completó lección: ${currentLesson.title}`,
        time: "Ahora mismo",
        tokens: lessonTokens,
        type: "completed",
      })
    ...
    // Update local storage for progress
    localStorage.setItem(
      `course-${courseId}-progress`,
      JSON.stringify({ hasStarted: true, currentLessonId: currentLesson.id }),
    )

    if (hasNext) {
      const nextLesson = allLessons[currentLessonIndex + 1]
      router.push(`/module/${courseId}/lesson/${nextLesson.id}`)
    } else if (currentLesson.type === "sandbox") {
      // If current lesson is sandbox and it's the last one, navigate to completion
      router.push(`/module/${courseId}/completion`)
    } else {
      // If it's the last lesson and not a sandbox, navigate to sandbox
      router.push(`/module/${courseId}/sandbox`)
    }
  }
  ...
  const handlePrevious = () => {
    if (hasPrevious) {
      ...
      router.push(`/module/${courseId}/lesson/${previousLesson.id}`)
    } else {
      // If it's the first lesson, go back to the module detail page
  ...
  if (!currentLesson) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-light-gray text-dark-gray">
        <p>Lección no encontrada.</p>
        <Link href={`/module/${courseId}`} className="text-deep-blue hover:underline mt-4">
          Volver al curso
        </Link>
      </div>
    )
    ...
    <div className="flex min-h-screen flex-col bg-light-gray">
      <header className="sticky top-0 z-50 w-full bg-header-blue shadow-sm">
        <div className="container flex h-20 items-center justify-between px-4 md:px-6">
          <Link href={`/module/${courseId}`} className="flex items-center gap-2">
        ...
        <div className="container px-4 md:px-6">
          {currentLesson.type === "video" && (
            <LessonVideo
              ...
              videoDuration="5:30" // Dummy duration
              onNext={handleNext}
          ...
          {currentLesson.type === "challenge" && (
            <ChallengeQuestion
              ...
              correctAnswerId={currentLesson.correctAnswerId || ""}
          ...
          {currentLesson.type === "sandbox" &&
            // Redirect to sandbox page if the current lesson is a sandbox
            // This page should not render the sandbox directly, but redirect
            router.push(`/module/${courseId}/sandbox`)}
        </div>
      </main>
    </div>
